...
import { validateLeaveRequest } from "../../validator";
...
function CreateLeaveRequest() {
  ...
  const handleFileChange = (e) => {
    ...
    if (fieldErrors.leaveFile) {
      setFieldErrors((prev) => ({ ...prev, leaveFile: "" }));
    }
  };
  ...
  const handleStudyTimeChange = (studyTimeId) => {
    ...
    // Clear field error when user selects study times
    if (fieldErrors.selectedStudyTimes) {
      setFieldErrors((prev) => ({ ...prev, selectedStudyTimes: "" }));
    }
    if (error) setError("");
  };

  const validateForm = () => {
    const validation = validateLeaveRequest(formData);

    if (!validation.success) {
      setFieldErrors(validation.errors);
      // Set general error message
      const firstError = Object.values(validation.errors)[0];
      setError(firstError || "กรุณากรอกข้อมูลให้ครบถ้วนและถูกต้อง");
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/leavereq", {
        state: {
          title: "ส่งคำร้องลาเรียนสำเร็จ",
          description: "ส่งคำร้องลาเรียนไปยังคุณครูประจำวิชาทุกคนสำเร็จ",
        },
      });
    } catch (error) {
              ...
              <div className="space-y-4">
                  ...
                  <select
                    ...
                    className={`w-full rounded-lg border ${
                      fieldErrors.leaveTypeId
                        ...
                        : "border-line"
                    } p-2.5 transition-colors focus:ring-2 focus:ring-primary focus:border-primary`}
                ...
                <div>
                  <label className="block text-sm font-medium text-text-color mb-2 flex items-center">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5 mr-1.5 text-text-color-alt"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
              ...
              {availableStudyTimes.length === 0 ? (
                ...
                <div
                  className={`border rounded-lg overflow-hidden transition-all ${
                    fieldErrors.selectedStudyTimes
                      ? "border-red-400 ring-1 ring-red-400"
                      : "border-line"
                  }`}
                >
                  {availableStudyTimes.map((studyTime) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          handleStudyTimeChange(studyTime.studyTimeId)
                        }
                      >
                        <div
                          className={`w-5 h-5 rounded-md border flex-shrink-0 flex items-center justify-center mr-3 transition-colors
                          ${
                            isSelected
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between flex-wrap gap-2">
                            <span
                              className={`font-medium ${
                                isSelected ? "text-primary" : "text-text-color"
                              }`}
                            >
                            ...
                            >
                              {formatTime(studyTime.timetable?.timeStart)} -{" "}
                              {formatTime(studyTime.timetable?.timeEnd)} น.
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-4 mt-1">
                            ...
                            {subject.teacher && (
                              <span className="text-sm text-text-color-alt">
                                อาจารย์: {subject.teacher.fName}{" "}
                                {subject.teacher.lName}
                              </span>
                            )}
                          </div>
                        </div>
              ...
              <textarea
                ...
                className={`w-full rounded-lg border ${
                  fieldErrors.leaveReason
                    ? "border-red-400 ring-1 ring-red-400"
                    : "border-line"
                } p-3 transition-colors focus:ring-2 focus:ring-primary focus:border-primary min-h-[150px]`}
              ...
              <div className="flex items-center justify-center w-full">
                <label
                  ...
                  border-2 border-dashed ${
                    ...
                    formData.leaveFile ? "bg-primary bg-opacity-5" : ""
                  }`}
                >
                    ...
                    {formData.leaveFile ? (
                      <>
                        ...
                        <p className="text-sm text-text-color font-medium">
                          {formData.leaveFile.name}
                        </p>
                        <p className="text-xs text-text-color-alt">
                          {(formData.leaveFile.size / 1024 / 1024).toFixed(2)}{" "}
                          MB
                        </p>
                      </>
                    ) : (
                      <>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-10 w-10 text-text-color-alt mb-2"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
