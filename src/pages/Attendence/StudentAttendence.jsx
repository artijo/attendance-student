...
function StudentAttendence() {
  ...
  const sortStudyTime = (studyTime) => {
    // => ฟังก์ชั่นสำหรับ sort ตารางเรียนที่ผ่านไปแล้วให้อยู่หลังสุด
    const now = DateTime.now();
    const passedOrNotPassStudingTimeArr = studyTime
      .filter((st) => {
        const stDate = DateTime.fromISO(
          `${now.toFormat("yyyy-MM-dd")}T${st.timetable.timeStart}`,
        ).setZone("Asia/Bangkok");
        const endDate = DateTime.fromISO(
          `${now.toFormat("yyyy-MM-dd")}T${st.timetable.timeEnd}`,
        ).setZone("Asia/Bangkok");
        return (
          (now > stDate && now > endDate) || (stDate > now && endDate > now)
        );
      })
      .sort((a, b) => {
        const aTime = DateTime.fromISO(a.studingTimeDate).setZone(
          "Asia/Bangkok",
        );
        const bTime = DateTime.fromISO(b.studingTimeDate).setZone(
          "Asia/Bangkok",
        );
  ...
  const ErrorAlertDialog = ({ message }) => {
    return (
      <div
        role="alert"
        className=" rounded-md border border-red-100 bg-red-100 p-4"
      >
        <div className="flex flex-col items-start relative">
          <div className="flex  items-start gap-4 mt-2">
            ...
            <div className="flex-1">
              <strong className="block font-medium text-red-600">
                เกิดข้อผิดพลาด
              </strong>
            </div>
          </div>
  ...
  const SucessfullAlertDialog = ({ message }) => {
    return (
      <div
        role="alert"
        className="rounded-md border border-green-100 bg-green-100 p-4"
      >
        <div className="flex flex-col items-start relative">
          <div className="flex items-start gap-4 mt-2">
            <span className="text-green-600">
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M12 2.25c5.385 0 9.75 4.365 9.75 9.75s-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12 6.615 2.25 12 2.25zm4.03 6.97a.75.75 0 0 0-1.06-1.06l-4.22 4.22-1.72-1.72a.75.75 0 1 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.06 0l4.75-4.75z"
                  clipRule="evenodd"
                />
              </svg>
            </span>
            <div className="flex-1">
              <strong className="block font-medium text-green-600">
                สำเร็จ
              </strong>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 border border-gray-200 rounded-xl shadow-md p-5">
          <div className="flex gap-2 items-center">
            ...
            <h5 className="font-bold text-lg text-slate-900">
              เกิดข้อผิดพลาดเกี่ยวการเข้าถึงต่ำแหน่งของผู้ใช้
            </h5>
          </div>
          <div>
            <h5 className=" font-bold text-lg text-slate-900">รายละเอียด</h5>
            <p className="text-sm indent-8 mt-2">
              ไม่สามารถเข้าถึงตำแหน่งที่ตั้งของคุณได้
              เนื่องจากคุณยังไม่ได้อนุญาต หรือระบบปฏิเสธการขอใช้ตำแหน่ง
              กรุณากดปุ่มด้านล่างเพื่อขออนุญาตใหม่อีกครั้ง
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="sm:max-w-md md:max-w-lg mx-auto p-2">
        <div className="mb-6">
          ...
          <p className="mt-3 text-gray-700">
            {"วัน" + weekDayToThaiString(currentDateTime.weekday)} ที่{" "}
            {currentDateTime.day} {getThaiMonth(currentDateTime.month)}{" "}
            {currentDateTime.year + 543}
          </p>
        </div>
        {location.latitude && location.longitude && (
                ...
                <div className="flex flex-col gap-3">
                  {currentStudingTime.map((item, index) => (
                    <EnrollmentCard
                      ...
                      enrollmentInfo={item}
                      callEnrollmentApi={() =>
                        callEnrollmentApi(item, location)
                      }
                      isError={error}
                      isLoading={isLoading}
                    />
              ...
              <div>
                ...
                <div className="mb-4 grid grid-cols-1 gap-4">
                  <p className="text-center text-gray-700">
                    ไม่มีคาบเรียนที่ดำเนินการขณะนี้
                  </p>
                </div>
              </div>
            )}
            {passedOrNotPassStudingTime.length > 0 && (
              <div>
                <div className="flex gap-2 items-center mb-3">
                  ...
                  <h5 className="font-bold text-lg text-slate-900">
                    คาบเรียนทีผ่านไปแล้วและยังไม่ถึง
                  </h5>
                </div>
                <div className="flex flex-col gap-3">
