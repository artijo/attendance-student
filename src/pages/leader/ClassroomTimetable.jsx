  ...
  const fetchTimetable = async () => {
    ...
    try {
      const response = await axios.get(
        `${HOSTNAME}/s/leader/classrooms/${classId}/timetable`
      );
      ...
      if (err.response && err.response.status === 404) {
        setError("ไม่พบข้อมูลตารางเรียนสำหรับห้องเรียนนี้");
      } else {
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-4 bg-gray-50 p-4 rounded-lg flex justify-between items-center">
        <div>
          <p className="text-gray-700">
            ...
            {formatDateToThai(currentDateTime.toISODate())}
          </p>
      ...
      {loading ? (
        ...
        <div className="bg-red-50 p-4 rounded-lg text-center">
          ...
          <button onClick={fetchTimetable} className="mt-2 text-sm underline">
            ลองอีกครั้ง
          </button>
        </div>
                ...
                <div className="p-4">
                  <div className="flex items-center mb-3">
                    ...
                    <span className="text-gray-700">
                      {formatTimeThai(entry.timeStart)} -{" "}
                      {formatTimeThai(entry.timeEnd)}
                      <span className="text-gray-500 text-xs ml-2">
                        (สาย {formatTimeThai(entry.timeLate)})
                      </span>
                    </span>
                  </div>
