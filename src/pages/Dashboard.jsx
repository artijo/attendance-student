  ...
  const handleAttendance = async (enrollmentInfo) => {
    ...
    try {
      const response = await axios.post(
        `${HOSTNAME}/s/attendence/enrollment`, 
        { 
      ...
      {locationStatus.permission !== "granted" && (
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-lg p-4 shadow-sm">
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-amber-800">การเข้าถึงตำแหน่งที่ตั้งถูกปิดอยู่</h3>
      ...
      <div className="mb-8">
        ...
        {loading ? (
                ...
                <div className="bg-primary p-4 text-white">
                  <div className="flex justify-between items-center">
                    <h2 className="text-lg font-bold">
                      {classItem.timetable.subject.subCode} - {classItem.timetable.subject.subNameThai}
                    </h2>
                ...
                <div className="p-4">
                  <div className="flex items-center mb-3">
                    ...
                    <span className="text-gray-700">
                      {formatTime(classItem.timetable.timeStart)} - {formatTime(classItem.timetable.timeEnd)} น.
          ...
          <div className="bg-gray-50 rounded-lg p-8 text-center border border-gray-200 shadow-sm">
            ...
            <h3 className="text-lg font-medium text-gray-900">ไม่มีรายวิชาที่กำลังเรียนในขณะนี้</h3>
            <p className="mt-2 text-gray-500">
              ไม่มีรายวิชาที่กำลังเรียนอยู่ในขณะนี้ หรือคุณอาจจะต้องเปิดการเข้าถึงตำแหน่งที่ตั้ง
            </p>
          </div>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {upcomingClasses.map((classItem) => {
              ...
              const isCurrent = isCurrentlyInClass(classItem.timetable.timeStart, classItem.timetable.timeEnd);
                ...
                >
                  <div className={`${bgColor} p-3 text-white`}>
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`${statusBgColor} px-2 py-0.5 rounded-full text-xs font-bold ${statusColor} whitespace-nowrap shadow-sm`}>
                        {textStatus}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-3">
                    <div className="flex items-center text-sm mb-2">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-4 w-4 ${isPast ? 'text-gray-400' : isCurrent ? 'text-primary' : 'text-secondary'} mr-1 flex-shrink-0`}
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      >
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                      </svg>
                      <span className={`${isPast ? 'text-gray-500' : 'text-gray-700'}`}>
                        {formatTime(classItem.timetable.timeStart)} - {formatTime(classItem.timetable.timeEnd)} น.
                      </span>
                    </div>
                    ...
                    <div className="flex items-center text-sm">
                      ...
                      <span className={`${isPast ? 'text-gray-500' : 'text-gray-700'} truncate`}>
                        คุณครู {classItem.timetable.subject.teacher.fName} {classItem.timetable.subject.teacher.lName}
                      </span>
                    </div>
                  </div>
